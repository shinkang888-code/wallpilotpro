...
export function DartLabPanel() {
  ...
  useEffect(() => {
    void getDartLabStatus({ data: {} }).then((s) => setOpendartOk(s.opendartConfigured));
  }, []);
  ...
  const runAnalysis = async (symbol: string) => {
    ...
    try {
      ...
      setQuery(code);
      setTab("ai");
      toast.success(t("dart_analysis_done"));
    } catch (e) {
      if (seq !== requestSeq.current) return;
    ...
    <div className="space-y-8">
      ...
      {result ? (
        <section className="space-y-5">
          ...
          {tab === "ai" && (
            <div className="max-h-[min(70vh,560px)] overflow-y-auto rounded-2xl border border-hairline bg-white p-5">
              <pre className="whitespace-pre-wrap text-xs leading-relaxed text-foreground sm:text-sm">
