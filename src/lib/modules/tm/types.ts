export type TmJobMode = "backtest" | "train" | "regime_label" | "regime_test";
export type TmJobStatus = "queued" | "running" | "completed" | "failed" | "degraded";
export type TmJobSource = "trademaster" | "fallback";
export type TmJobPhase = "idle" | "train" | "test" | "done";
export type TmRunMode = "quick" | "full";

export type TmRlMetrics = {
  sharpe: number;
  totalReturnPct: number;
  maxDrawdownPct: number;
  winRatePct: number;
  trades: number;
};

export type TmEquityPoint = {
  date: string;
  value: number;
};

export type TmRlJob = {
  id: string;
  userId: string;
  mode: TmJobMode;
  runMode: TmRunMode;
  task: string;
  dataset: string;
  agent: string;
  tickers: string[];
  status: TmJobStatus;
  source: TmJobSource;
  phase: TmJobPhase;
  progressPct: number;
  progressMessage: string | null;
  trademasterSessionId: string | null;
  metrics: TmRlMetrics;
  equityCurve: TmEquityPoint[];
  chartNote: string | null;
  errorMessage: string | null;
  createdAt: string;
  completedAt: string | null;
};

export type TmWorkerStatus = {
  configured: boolean;
  online: boolean;
  latencyMs: number | null;
};

export type TmLabPresets = {
  tasks: { id: string; label: string }[];
  datasets: { id: string; label: string }[];
  agents: { id: string; label: string }[];
  worker: TmWorkerStatus;
};
