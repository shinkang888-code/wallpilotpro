...
export type AgentDeskPipelineResult = {
...
export async function buildAnalystReports(
  ...
  const prompt = `You are four WallPilot analyst agents (Market, Fundamentals, News, Social/Sentiment) writing concise desk reports.
Use ONLY the data below. Each report: 3-5 sentences, plain prose, no markdown headers.

Ticker: ${snapshot.ticker} (${snapshot.name}) 쨌 ${snapshot.market}
Price: ${snapshot.price} ${snapshot.currency} 쨌 30D: ${snapshot.change30dPct.toFixed(1)}%
Intrinsic: ${valuation.intrinsicValue} 쨌 MOS: ${valuation.marginOfSafetyPct.toFixed(1)}%
PEG: ${valuation.pegRatio ?? "N/A"} 쨌 Quant: ${valuation.quantitativeGrade}
...
export function renderAgentDeskMarkdown(input: {
  ticker: string;
  name: string;
  analysts: AnalystReports;
  debate: DebateVerdict;
  ...
  return `# ${name} (${ticker}) — WallPilot Agent Desk Report
...
async function runTraderAndPortfolio(
  ...
  const prompt = `You are a WallPilot Trader + Portfolio Manager (single JSON response).
...
/** WallPilot Agent Desk pipeline (TS primary, optional Python sidecar). */
export async function runAgentDeskPipeline(
  ctx: WallStreetReportContext,
  geminiApiKey?: string | null,
): Promise<AgentDeskPipelineResult> {
  ...
  const markdown = renderAgentDeskMarkdown({
