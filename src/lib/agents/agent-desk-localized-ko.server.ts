...
function fallbackLocalizedKo(
  ...
  return {
    analysts: {
      market: `${report.name}(${report.ticker}) 현재가 ${price}. ${report.technicalLabel ? `기술적 상태: ${report.technicalLabel}. ` : ""}30일 수익률·이동평균·거래량을 기준으로 단기 추세를 점검합니다.`,
      fundamentals: `PEG ${report.lynch.pegRatio.toFixed(2)}, ROIC ${report.greenblatt.roic}%, 안전마진 ${report.combined.marginOfSafetyPct.toFixed(1)}% 등 밸류에이션·퀀트 지표를 종합합니다. ${report.lynch.recommendation} · ${report.greenblatt.recommendation}`,
      news: analysts.news.includes("헤드라인") || /[가-힣]/.test(analysts.news)
        ? analysts.news
        : `최근 뉴스·공시 흐름: ${analysts.news}`,
      sentiment: `수급·심리: ${report.supply.label}. 종합 의견 ${report.combined.recommendation}. ${analysts.sentiment}`,
    },
    debate: {
      ...
      bearCase: `약세: 밸류에이션·거시 변동성 리스크. ${debate.bearCase}`,
      verdict: debate.verdict.startsWith("리서치") || /[가-힣]/.test(debate.verdict)
        ? debate.verdict
    ...
    trader: {
      ...
      reasoning: trader.reasoning.startsWith("매수") || /[가-힣]/.test(trader.reasoning)
        ? trader.reasoning
        : `매매 의견 ${trader.action}. ${trader.reasoning}`,
    ...
    risk_gate: {
      ...
      reason: riskGate.approved
        ? `리스크 점검 통과: ${riskGate.reason}`
        : `리스크 점검 차단: ${riskGate.reason}`,
...
export async function generateAgentDeskLocalizedKo(input: {
  ...
  const prompt = `You are a senior Korean equity research desk. Translate the TradingAgents pipeline output into natural Korean for UI cards.
